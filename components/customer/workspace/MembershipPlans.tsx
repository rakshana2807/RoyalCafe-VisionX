...
export default function MembershipPlans() {
  ...
  const [activePlanType, setActivePlanType] = useState<"wifi" | "daily">(
    "wifi"
  );

  const { wifiPass, setWifiPass, clearSelectedSeat } = useBooking();

  const isSelectedPass = (name: string) => {
    return wifiPass?.name === name;
  };
  ...
  const handleSelectPass = (
    ...
    if (!isAuthenticated()) {
      const msg = encodeURIComponent(
        "Please login to your RoyalCafeConnect account to select a WiFi pass."
      );
      router.push(`/login?redirect=/work-study&message=${msg}`);
      return;
    }
     ...
     * ---------------------------------------------------------
     *
     * If the user clicks the selected pass again,
     * take them to the booking page.
     */
    if (wifiPass?.name === name) {
      clearSelectedSeat(); // Important: Starting a NEW booking flow!
      router.push("/book");
      return;
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mx-auto text-center mb-8 animate-fade-in">
          ...
          <div className="inline-flex p-1 bg-[#FAF6F0] rounded-full border border-primary/10">
            ...
            <button
              ...
              className={`px-6 py-2 rounded-full text-xs font-bold transition-all duration-300 cursor-pointer ${activePlanType === "wifi"
        ...
        <div className="max-w-4xl mx-auto mb-10 bg-[#FAF6F0] border-l-4 border-[#5A2E0C] rounded-r-2xl p-5 sm:p-6 flex items-start gap-3.5 text-left card-shadow">
          ...
          <p className="text-xs sm:text-sm text-foreground/80 leading-relaxed font-sans font-medium">
            Every guest receives 30 minutes of Complimentary Wi-Fi with any
            purchase. For extended study sessions, choose a pass below.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          ...
          <div className="bg-[#FAF6F0]/60 p-8 sm:p-10 rounded-[28px] border border-primary/10 flex flex-col justify-between text-left card-shadow h-full">
            ...
            <div>
              ...
              <div className="text-center mb-6">
                ...
                <span className="text-3xl font-bold font-serif text-[#5A2E0C]">
                  ₹39
                </span>
            ...
            <button
              ...
              className={`w-full py-4 px-6 border border-transparent text-xs font-bold rounded-full text-white transition-all shadow-md cursor-pointer uppercase tracking-wider text-center flex items-center justify-center gap-1.5 ${isSelectedPass("Student Pass")
          ...
          <div className="bg-[#FAF6F0]/60 p-8 sm:p-10 rounded-[28px] border-2 border-[#5A2E0C]/30 flex flex-col justify-between text-left card-shadow h-full relative">
            ...
            <div>
              ...
              <ul className="space-y-3.5 mb-8">
                ...
                <li className="flex gap-2.5 items-center text-xs sm:text-sm font-semibold text-foreground/85">
                  ...
                  <span>
                    Unlimited 200 Mbps Fiber connection
                  </span>
                ...
                <li className="flex gap-2.5 items-center text-xs sm:text-sm font-semibold text-foreground/85">
                  ...
                  <span>
                    Noise-cancelling headphone rental
                  </span>
            ...
            <button
              ...
              className={`w-full py-4 px-6 border border-transparent text-xs font-bold rounded-full text-white transition-all shadow-md cursor-pointer uppercase tracking-wider text-center flex items-center justify-center gap-1.5 ${isSelectedPass("Work Pass")
                  ? "bg-[#8C4A21] hover:bg-[#3D2314]"
                  : "bg-[#EA5A0C] hover:bg-[#EA5A0C]/90"
                }`}
            >
