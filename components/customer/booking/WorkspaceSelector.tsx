...
export default function WorkspaceSelector() {
  ...
  const handleFormSubmit = async () => {
    ...
    if (!reservationDetails.resDate) {
      alert("Please complete workspace and time selection: Select a Reservation Date.");
      return;
    }
    if (!reservationDetails.arrivalTime.trim()) {
      alert("Please complete workspace and time selection: Select Arrival Start Time.");
      return;
    }
    ...
    try {
      ...
      const effectiveWorkspace = selectedSeat?.id || reservationDetails.tableType || "Workstation W-08";
      ...
      // Store in local storage for profile & admin views
      ...
      router.push(`/payment?amount=${grandTotal.toString()}`);
    } finally {
